...
使用两张图片时，`reference_image` 是**图 1**，即决定输出宽高比的编辑目标；`reference_image_2` 是附加外观参考。提示词中用 `<image1>`、`<image2>` 指代它们。采样器的 `latent_image` 必须接本编码节点的 **`latent`**；另接宽高比不匹配的空 latent 会使编辑内容漂移。控制图也应按目标画布的宽高比准备，避免拉伸。姿态等结构控制不保证把另一个人的五官或服装精准迁移过来。

| 输入 | 应连接的图片 |
| --- | --- |
| `control_image` | 已预处理的边缘、深度、姿态等控制图 |
| `reference_image` | 图 1：要编辑的原图，决定输出画布 |
| `reference_image_2` / `_3` | 附加外观或风格参考图 |
